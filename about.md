---
title: "About"
layout: page
permalink: /about/
---

## About the Group

The **Harvard Mathematics Departmental Group** is a student-run organization within the Harvard University Mathematics Department. Our mission is to:

1. Build a strong and supportive mathematical community.
2. Facilitate opportunities for collaboration across research groups and interests.
3. Support professional and academic development.
4. Organize events that create space for exploration, discussion, and social connection.

## Activities

The group organizes a wide variety of events, including:

- **Seminars and reading groups**
- **Graduate social events**
- **Workshops and academic support sessions**
- **Interdisciplinary meetups with other departmental or GSAS groups**

## Leadership (2025–2026)

- **[Name 1]** – Co-organizer
- **[Name 2]** – Co-organizer
- **[Name 3]** – Treasurer / Communications
- **[Name 4]** – Faculty Advisor *(optional)*

If you are interested in helping organize or propose an event, please email:
**hggms@math.harvard.edu** (or whatever address you end up using).

## Membership

Membership is open to all students in the Harvard Mathematics Department with an interest in mathematics, regardless of background or specialization.
